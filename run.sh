...
docker run -d --name prs-bridge --restart unless-stopped --network openclaw-kx5x_default -e JARVIS_BRIDGE_SECRET="$SECRET" -e HOOKS_TOKEN="$HOOKS_TOKEN" -e HOOKS_PATH="/hooks-prs-abertas" -e GATEWAY_URL="http://openclaw-kx5x-openclaw-1:18789" --label "traefik.enable=true" --label "traefik.http.routers.prs-bridge.rule=Host(\`prs-bridge.davissync.com\`)" --label "traefik.http.routers.prs-bridge.entrypoints=websecure" --label "traefik.http.routers.prs-bridge.tls.certresolver=letsencrypt" --label "traefik.http.services.prs-bridge.loadbalancer.server.port=8791" prs-bridge
